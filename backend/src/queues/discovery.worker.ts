...
export class DiscoveryWorker extends WorkerHost {
  ...
  async process(job: BullJob<DiscoveryJobPayload>): Promise<any> {
    ...
    return await context.with(parentCtx, async () => {
      return await tracer.startActiveSpan('DiscoveryWorker.process', async (span) => {
        ...
        span.setAttribute('job.page', page);
        ...
        try {
          ...
          span.setAttribute('job.raw_scraped_count', rawScrapedJobs.length);
          ...
          if (rawScrapedJobs.length === 0) {
            ...
            await this.matchingQueue.add('evaluate', injectTraceContext(job.data));
          ...
          for (const rawJob of rawScrapedJobs) {
            await this.validationQueue.add('validate-job', injectTraceContext({
              runId,
