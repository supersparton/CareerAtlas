...
2. **NEVER hardcode API keys** in any TypeScript file. Always use `process.env.KEY_NAME` or ConfigService.
...
1. **No silent failures.** Every API call must have a `try/catch` block with a descriptive `Logger` message.
...
3. **Never change `seen_jobs.json` structure.** It must always be a flat JSON array of strings. Breaking this format breaks the deduplication engine.
...
1. **Test before committing.** Always run `npm run build` and `npm start` in the backend directory.
...
1. **Use LLM caching** (`processed_jobs.json`) to skip scoring previously evaluated jobs across runs.
...
3. **Limit LLM context to `page_text[:25000]`** characters per scrape. Sending full pages causes slow, costly responses.
...
├── backend/          ← NestJS application source directory.
├── profile.txt       ← User career target. User-edited only.
├── seen_jobs.json    ← Agent memory (matched). Agent-managed only.
├── processed_jobs.json ← Agent cache (evaluated). Agent-managed only.
